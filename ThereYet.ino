...
// overly simple timer
// call with zero to reset timer immediately
bool thereYet(unsigned long ticksToWait)
{
  static unsigned long ticksAtLastTrigger = 0;
  unsigned long ticksNow = millis();
  bool timesUp = false;

  unsigned long ticksSince = ticksNow - ticksAtLastTrigger;
  if (ticksSince >= ticksToWait)
  {
    // time's up
    ticksAtLastTrigger = ticksNow;
    timesUp = true;
  }
